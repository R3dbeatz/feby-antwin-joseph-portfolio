...
const Experience = () => {
  ...
  const lineColors = textParts.map((line, lineIndex) => {
    ...
    return chars.map((_, charIndex) => {
      ...
      return useTransform(scrollYProgress, [charStart, charEnd], isFirstLine ? ['#333333', '#eb5939'] : ['#333333', '#aa9e8b']);
    });
  });
  return <section ref={sectionRef} className="min-h-screen flex items-center justify-center bg-dark py-20 relative" id="expertise" aria-label="Experience Section">
      {/* Particle background */}
      <div className="absolute inset-0 w-full h-full overflow-hidden">
        <Particles particleColors={['#eb5939', '#aa9e8b', '#ffffff']} particleCount={800} particleSpread={10} speed={0.1} particleBaseSize={80} moveParticlesOnHover={true} alphaParticles={false} disableRotation={false} />
      </div>
        ...
        once: true
          ...
          <div className="max-w-[1440px] mx-auto px-4">
            <div className="leading-[1.1] tracking-tight font-semibold text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl text-left">
              {textParts.map((line, lineIndex) => <div key={`line-${lineIndex}`} className="block mb-2">
                  {line.split('').map((char, charIndex) => <motion.span key={`line-${lineIndex}-char-${charIndex}`} style={{
